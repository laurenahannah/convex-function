function [alpha, beta, abStruct] = CAPfast(x,y,convexFlag,logMult,knotNum)

% CAPr runs the CAP algorithm on input data x,y to create a piecewise
% linear convex or concave function with a fixed number of random
% projections for search directions
%
% Inputs:
...
%                     observations per partition; default is 5
%
% Outputs:
...
GCVvec = [];
GCVvec = [GCVvec; sum(res.^2)/(n*(1-(d+1)/n)^2)];
...
while(keepGoing)
    ...
    for k = 1:K
        ...
        if (nk >= max(2*n/(logMult*log(n)), 4*(d+1)))
            ...
            for j = 1:randDir
                ...
                minVec = min(xHat*dirVec);
                maxVec = max(xHat*dirVec);
                if (maxVec - minVec < 0.0001) % We need to jitter
                    xHat(:,j) = xHat(:,j) + 0.001*randn(nk,1);
                    minVec = min(xHat*dirVec);
                    maxVec = max(xHat*dirVec);
                end
                ...
                for ell = 2:knotNum
                    ...
                    % enough data
                    xHat1Ind = find(xHat*dirVec <= knotMat(ell));
                    xHat2Ind = find(xHat*dirVec > knotMat(ell));
                    n1 = length(xHat1Ind);
                    n2 = length(xHat2Ind);
                    if ((n1 > minNode)&&(n2 > minNode))
                        ...
                        idListNew = idListOld;
                        idHi = find(x*dirVec > knotMat(ell));
                ...
                if (counter == 1) % We need to split by the median
                    xMed = median(xHat*dirVec);
                    xHat1Ind = find(xHat*dirVec <= xMed);
                    xHat2Ind = find(xHat*dirVec > xMed);
                    n1 = length(xHat1Ind);
                    n2 = length(xHat2Ind);
                    if ((n1 > 0)&&(n2 > 0))
                        % Then we can compute betas
    ...
    if (length(BICvec) >= 3)
        qqq = length(BICvec);
        if ((BICvec(qqq)> BICvec(qqq-1) )&&(BICvec(qqq) > BICvec(qqq-2)))
            keepGoing = false;
        end
    end
end
